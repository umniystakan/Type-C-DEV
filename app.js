...
class TypeCMessenger {
    ...
    async loadWeather(force = false) {
        ...
        if (!force) {
            const cachedWeather = sessionStorage.getItem('weather_data_v556');
            const now = Date.now();
            if (cachedWeather) {
                ...
                if (data.current_weather) {
                    ...
                    if (data.daily && data.daily.weathercode) {
                        for (let i = 1; i <= 3; i++) {
                            ...
                            const fDay = this.getForecastDayName(data.daily.time[i]);

                            forecastHtml += `
                                <div class="forecast-item" title="${fInfo.label}">
                                    <div class="forecast-day">${fDay}</div>
                                    <div class="forecast-icon ${fInfo.class}">${fInfo.icon}</div>
                                    <div class="forecast-temp">${fMax}° / ${fMin}°</div>
                                </div>
                            `;
                        }
                    } else {
                        console.warn('[WEATHER] Daily data missing');
                        forecastHtml = '<p class="empty-text">Прогноз временно недоступен</p>';
                    }
                    ...
                    sessionStorage.setItem('weather_data_v556', JSON.stringify({
                        timestamp: Date.now(),
                        html: weatherHtml
                    }));
                }
