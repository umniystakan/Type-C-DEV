...
class TypeCMessenger {
    ...
    addMessageToTimeline(event, scrollToBottom = true, prepend = false) {
        try {
            ...
            // Build actions menu (delete for self / for everyone)
            const canRedact = isMe || isAdmin;
            const safeEventId = eventId && !eventId.startsWith('~') ? eventId : null;
            const deleteForAllBtn = safeEventId && canRedact
                ? `<button class="msg-action-btn" data-action="delete-all" title="Удалить для всех"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6l-1 14H6L5 6"></path><path d="M10 11v6"></path><path d="M14 11v6"></path><path d="M9 6V4h6v2"></path></svg> Для всех</button>`
                : '';
            const deleteForMeBtn = safeEventId
                ? `<button class="msg-action-btn" data-action="delete-me" title="Удалить для меня"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 9l-5 5-5-5"></path></svg> Для меня</button>`
                : '';
            const actionsHtml = (deleteForAllBtn || deleteForMeBtn)
                ? `<div class="msg-actions">${deleteForAllBtn}${deleteForMeBtn}</div>`
                : '';
            ...
            // Wire up message action buttons (delete for me / for all)
    ...
    async loadWeather(force = false) {
        ...
        if (!force) {
            ...
            const now = Date.now();
            if (cachedWeather) {
                const { timestamp, html } = JSON.parse(cachedWeather);
                    ...
                    const weatherHtml = `
                        <div class="weather-info">
                            ...
                            <div class="weather-data">
                                ...
                                <div class="weather-location">Ваше местоположение</div>
                            </div>
                        </div>
                    `;
                    container.innerHTML = weatherHtml;
                    sessionStorage.setItem('weather_data', JSON.stringify({
                        timestamp: Date.now(),
                        html: weatherHtml
                    }));
                }
    ...
    getWeatherInfo(code) {
        const codes = {
            0: { label: 'Ясно', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></svg>' },
            1: { label: 'Преимущественно ясно', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v2"/><path d="m4.93 4.93 1.41 1.41"/><path d="M20 12h2"/><path d="m19.07 4.93-1.41 1.41"/><path d="M15.947 12.65a4 4 0 0 0-5.925-4.128"/><path d="M13 22H7a5 5 0 1 1 4.9-6H13a3 3 0 0 1 0 6Z"/></svg>' },
            2: { label: 'Переменная облачность', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v2"/><path d="m4.93 4.93 1.41 1.41"/><path d="M20 12h2"/><path d="m19.07 4.93-1.41 1.41"/><path d="M15.947 12.65a4 4 0 0 0-5.925-4.128"/><path d="M13 22H7a5 5 0 1 1 4.9-6H13a3 3 0 0 1 0 6Z"/></svg>' },
            3: { label: 'Пасмурно', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17.5 19x2a4.5 4.5 0 0 0 0-9 4.5 4.5 0 0 0-8.5 1.5c-1.5 0-3 1.5-3 3s1.5 3 3 3Z"/><path d="M12 12V3"/><path d="M16 12V3"/><path d="M8 12V3"/></svg>' },
            45: { label: 'Туман', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14"/><path d="M5 16h14"/><path d="M5 8h14"/><path d="M5 20h14"/></svg>' },
            48: { label: 'Туман', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14"/><path d="M5 16h14"/><path d="M5 8h14"/><path d="M5 20h14"/></svg>' },
            61: { label: 'Слабый дождь', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 16.58A5 5 0 0 0 18 7h-1.26A8 8 0 1 0 4 15.25"/><path d="M8 19v2"/><path d="M12 21v2"/><path d="M16 19v2"/></svg>' },
            63: { label: 'Дождь', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 16.58A5 5 0 0 0 18 7h-1.26A8 8 0 1 0 4 15.25"/><path d="M8 19v2"/><path d="M12 21v2"/><path d="M16 19v2"/></svg>' },
            65: { label: 'Сильный дождь', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 16.58A5 5 0 0 0 18 7h-1.26A8 8 0 1 0 4 15.25"/><path d="M8 19v2"/><path d="M12 21v2"/><path d="M16 19v2"/></svg>' },
            71: { label: 'Снег', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 17.58A5 5 0 0 0 18 8h-1.26A8 8 0 1 0 4 16.25"/><path d="M8 22h.01"/><path d="M12 22h.01"/><path d="M16 22h.01"/></svg>' },
            95: { label: 'Гроза', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 16.9A5 5 0 0 0 18 7h-1.26a8 8 0 1 0-11.62 9"/><polyline points="13 11 11 17 15 17 13 23"/></svg>' },
        };
        return codes[code] || { label: 'Погода', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v20M2 12h20"/></svg>' };
    }
