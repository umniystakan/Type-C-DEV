...
class TypeCMessenger {
    ...
    async loadWeather(force = false) {
        ...
        if (!force) {
            const cachedWeather = sessionStorage.getItem('weather_data_v1050');
            const now = Date.now();
            if (cachedWeather) {
        ...
        navigator.geolocation.getCurrentPosition(async (position) => {
            ...
            try {
                ...
                if (data.current_weather) {
                    ...
                    sessionStorage.setItem('weather_data_v1050', JSON.stringify({
                        timestamp: Date.now(),
                        html: weatherHtml,
    ...
    getWeatherInfo(code) {
        const codes = {
            ...
            3: {
                label: 'Пасмуррно', class: 'weather-cloud', icon: `
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="cloud-icon">
