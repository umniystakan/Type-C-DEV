...
class TypeCMessenger {
    ...
    async loadWeather(force = false) {
        ...
        if (!force) {
            const cachedWeather = sessionStorage.getItem('weather_data_v435');
            const now = Date.now();
            if (cachedWeather) {
        ...
        navigator.geolocation.getCurrentPosition(async (position) => {
            ...
            try {
                ...
                if (data.current_weather && data.daily) {
                    ...
                    sessionStorage.setItem('weather_data_v435', JSON.stringify({
                        timestamp: Date.now(),
                        html: weatherHtml
                    }));
                }
        ...
        const codes = {
            ...
            2: {
                ...
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="cloud-icon">
                    <path d="M17.5 19x2a4.5 4.5 0 0 0 0-9 4.5 4.5 0 0 0-8.5 1.5c-1.5 0-3 1.5-3 3s1.5 3 3 3Z" fill="rgba(255,255,255,0.1)"/>
                </svg>` },
            3: {
