...
class TypeCMessenger {
    ...
    async loadWeather(force = false) {
        ...
        if (!force) {
            const cachedWeather = sessionStorage.getItem('weather_data_v777');
            const now = Date.now();
            if (cachedWeather) {
        ...
        navigator.geolocation.getCurrentPosition(async (position) => {
            ...
            const url = `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current_weather=true&daily=weathercode,temperature_2m_max,temperature_2m_min&timezone=auto&forecast_days=3`;
            ...
            try {
                ...
                if (data.current_weather) {
                    ...
                    // Process Forecast (Today + Next 2nd/3rd day)
                    let forecastHtml = '';
                    if (data.daily && data.daily.weathercode && data.daily.weathercode.length > 0) {
                        // Show Today, Tomorrow, and Day After
                        for (let i = 0; i < 3; i++) {
                            ...
                            forecastHtml += `
                                <div class="forecast-item" title="${fInfo.label}">
                                    ...
                                    <div class="forecast-icon ${fInfo.class}">${fInfo.icon.replace('weather-icon', 'forecast-mini-icon')}</div>
                                    <div class="forecast-temp">${fMax}° / ${fMin}°</div>
                                </div>
                            `;
                        }
                    } else {
                        console.warn('[WEATHER] Daily data missing or empty');
                        forecastHtml = '<div style="grid-column: 1/-1; text-align: center; opacity: 0.5; font-size: 11px;">Прогноз недоступен</div>';
                    }

                    const weatherHtml = `
                        <div class="weather-composite">
                            <div class="weather-current-row">
                                <div class="weather-info ${info.class}">
                    ...
                    sessionStorage.setItem('weather_data_v777', JSON.stringify({
                        timestamp: Date.now(),
                        html: weatherHtml
                    }));
                }
        ...
        const codes = {
            0: {
                ...
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="sun-icon">
                    <circle cx="12" cy="12" r="5" fill="rgba(255, 204, 0, 0.2)"/>
                    <g class="sun-rays">
            ...
            1: {
                ...
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="cloud-sun-icon">
                    ...
                    <path d="M13 22H7a5 5 0 1 1 4.9-6H13a3 3 0 0 1 0 6Z" fill="rgba(255,255,255,0.05)"/>
                </svg>` },
            2: {
                ...
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="cloud-icon">
                    <path d="M17.5 19a4.5 4.5 0 0 0 0-9 4.5 4.5 0 0 0-8.5 1.5c-1.5 0-3 1.5-3 3s1.5 3 3 3h8.5Z" fill="rgba(255,255,255,0.1)"/>
                </svg>` },
            3: {
                label: 'Пасмурно', class: 'weather-cloud', icon: `
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="cloud-icon">
                    <path d="M1 12.5a5 5 0 1 1 9.9 0h.1a3 3 0 0 1 0 6h-10a5 5 0 0 1 0-10Z" fill="rgba(255,255,255,0.1)"/>
                </svg>` },
            45: {
                label: 'Туман', class: 'weather-fog', icon: `
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="fog-icon">
                    <path d="M5 12h14"/><path d="M5 16h14"/><path d="M5 8h14"/><path d="M5 20h14"/>
                </svg>` },
            48: {
                label: 'Туман', class: 'weather-fog', icon: `
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="fog-icon">
            ...
            95: {
                ...
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="thunder-icon">
                    ...
                    <polyline points="13 11 11 17 15 17 13 23" fill="rgba(255,255,0,0.5)" stroke="yellow"/>
                </svg>` },
        };
        return codes[code] || {
